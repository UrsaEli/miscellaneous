...
def aes_encoder(block, key):
    ...
    return string_to_bits(str(ecb.encrypt(bytes(block, encoding="latin-1")), encoding="latin-1"))
...
def cipher_block_chaining(plaintext, key, init_vec, block_size, block_enc):
    ...
    ciphertext = []
    previousCiphertextBlock = init_vec
    
    for i in range(((len(plaintext) - 1) // block_size) + 1):
        startIndex = i * block_size
        endIndex = min((i + 1) * block_size, len(plaintext))
        plaintextBlock = plaintext[startIndex:endIndex]
        xorBlock = [p ^ c for p, c in zip(plaintextBlock, previousCiphertextBlock)]
        ciphertextBlock = block_enc(xorBlock, key)
        
        ciphertext.extend(ciphertextBlock)
