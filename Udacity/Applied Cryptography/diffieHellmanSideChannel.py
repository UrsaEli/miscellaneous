...
#
# HW3-5 Version 1 -- Challenge question
#
...
def generate_possible_keys(multiplications):
    """
    Returns the set of all keys that would require the specified
    number of multiplications when used in the Diffie-Hellman protocol.
    """
    keys = set()
    
    starting_key = [1] + [0] * multiplications
    
    generate_possible_key_bits(keys, starting_key)
    
    return [bits_to_int(seq_to_bits(key)) for key in keys]

    
def generate_possible_key_bits(keys, keybits):
    """
    Recursive helper for generate_possible_keys
    """
    bits_string = display_bits(keybits)
    # Don't waste time computing redundant keys
    if bits_string in keys:
        return
    
    keys.add(bits_string)
    
    for i in range(len(keybits) - 1):
        # Replace two zeroes with a one; the resulting key will still require the same
        # amount of multiplications
        if keybits[i] == 0 and keybits[i + 1] == 0:
            new_keybits = keybits[0:i] + [1] + keybits[i + 2:]
            generate_possible_key_bits(keys, new_keybits)
...
plaintext = "attack at dawn" # Your answer here 
...
if __name__ == "__main__":
    ...
    keys = generate_possible_keys(n_multiplications)
    print("Trying", len(keys), "keys")
    
    for x_a in keys:
        key = convert_to_bits(mod_exp(g_b, x_a, p))
        if len(ciphertext) > len(key):
            continue
            
        possible_plaintext_bits = decode(ciphertext, key)
        possible_plaintext_string = bits_to_string(possible_plaintext_bits)
        if is_valid(possible_plaintext_string):
            print("Found potential plaintext:", possible_plaintext_string)
            if test(x_a, possible_plaintext_string):
